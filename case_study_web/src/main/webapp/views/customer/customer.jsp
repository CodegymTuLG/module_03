...
<div class = "row" style="height: 81%">
    ...
    <%--List Customer--%>
    <div class="col-10"> <h1>Customer</h1>
        <form action="/customer?action=search" method="post">
            <input name="searchName" placeholder="input name" value="${searchName}">
            <input name="searchAddress" placeholder="input address" value="${searchAddress}">
            <select name="searchType" id="searchType">
                <option value=""></option>
            <c:forEach var="customerType" items="${customerTypeMasters}" varStatus="status">
                <option value="${customerType.getType()}">${customerType.getType()}</option>
            </c:forEach>
            </select>
        ...
        <a href="/customer?action=create"><button class="btn btn-primary">Register new customer</button></a><br><br>
        </div>
        <table class="table table-info" style="width: 98%;">
            ...
            <c:forEach var="customer" items="${customerList}" varStatus="status">
                <tr>
                    ...
                    <td>${customer.isSex()}</td>
                    <td>${customer.getCardid()}</td>
                    <td>${customer.getPhonenumber()}</td>
                    <td>${customer.getEmail()}</td>
                    <td>${customer.getAddress()}</td>
                    <td>${customer.getCustomerTypeMaster().getType()}</td>
                    <td>
                        <form action="/customer?action=edit" method="post">
                            <input type="hidden" name="editCustomerId" id="editCustomerId" value="${customer.getCustomer_id()}">
                            <button type="submit" class="btn btn-info">Edit</button>
                        </form>
                    </td>
                    <td>
                            <!-- Button trigger modal -->
                            <button type="button" onclick="deleteEmployee('${customer.getCustomer_id()}','${customer.getName()}')" class="btn btn-danger" data-bs-toggle="modal" data-bs-target="#exampleModal">
                                Delete
                            </button>
                    </td>
                </tr>
        ...
        <div class="modal-content">
            ...
            <form action="/customer?action=delete" method="post">
                <div class="modal-body">
                    <input readonly type="hidden" name="deleteCustomerId" id="deleteCustomerId">
