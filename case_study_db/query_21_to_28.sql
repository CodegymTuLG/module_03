...
sp_payment varchar(45), 
sp_employee_id int,
sp_customer_id int,
sp_service_id int)
begin

insert into contract(startdate, enddate, deposit, payment, employee_id, customer_id, service_id)
values
(sp_startdate, sp_enddate, sp_deposit, sp_payment, 
...
for each row
begin
insert into log(table_action, action, message) value('contract', 'delete', 'contract table have '+(select count(contract_id) from contract)+' records after delete');
end//
delimiter ;
...
-- Ngày kết thúc hợp đồng phải lớn hơn ngày làm hợp đồng ít nhất là 2 ngày. Nếu dữ liệu hợp lệ thì cho phép cập nhật, nếu dữ liệu không hợp lệ thì in ra thông báo “Ngày kết thúc hợp đồng phải lớn hơn ngày làm hợp đồng ít nhất là 2 ngày” trên console của database.
